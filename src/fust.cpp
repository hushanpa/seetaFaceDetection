      ...
      for (int32_t j = 0; is_loaded && j < hierarchy_size; j++) {
        ...
        for (int32_t k = 0; is_loaded && k < num_stage; k++) {
          ...
          is_loaded = !model_file.fail() &&
            reader->Read(&model_file, classifier.get());	//read a classifer
          if (is_loaded) {
            model_.push_back(classifier);					//save classifier in model_
            std::shared_ptr<seeta::fd::FeatureMap> feat_map;
            if (cls2feat_idx_.count(classifier_type) == 0) {	// get the first classifier of LAB or MLP
              feat_map_.push_back(CreateFeatureMap(classifier_type));	//create feature map
        ...
        model_file.read(reinterpret_cast<char*>(&num_wnd_src), sizeof(int32_t));	//read num_wnd_src of wnd_src_id   -1,-1,-1,3,1
        if (num_wnd_src > 0) {
          wnd_src_id_.back().resize(num_wnd_src);
          for (int32_t k = 0; k < num_wnd_src; k++) {
            model_file.read(reinterpret_cast<char*>(&(wnd_src_id_.back()[k])),		//read wnd_src_id array
              sizeof(int32_t));
          }
        }
      }
...
std::vector<seeta::FaceInfo> FuStDetector::Detect(
    ...
    for (int32_t y = 0; y <= max_y; y += slide_wnd_step_y_) {
      ...
      for (int32_t x = 0; x <= max_x; x += slide_wnd_step_x_) {
		...
		for (int32_t i = 1; i < 2; i++) {
          if (model_[i]->Classify(&score)) {				//3 parallel classifiers
			  result = 1;
            wnd_info.score = static_cast<double>(score);
            proposals[i].push_back(wnd_info);				//3 proposals arrays
			break;
          }
        }
  ...
  //num_hierarchy_ = 3;
  for (int32_t i = 1; i < num_hierarchy_; i++) {	//Coarse MLP and Fine MLP
    buf_idx.resize(hierarchy_size_[i]);
    for (int32_t j = 0; j < hierarchy_size_[i]; j++) {		//1 hierarchy
      int32_t num_wnd_src = static_cast<int32_t>(wnd_src_id_[cls_idx].size());		//3 and 1
      ...
      for (int32_t k = 0; k < num_stage_[cls_idx]; k++) {			//2 satges of Coarse MLP and 1 stage of Fine MLP
        int32_t num_wnd = static_cast<int32_t>(proposals[buf_idx[j]].size());
        std::vector<seeta::FaceInfo> & bboxes = proposals[buf_idx[j]];
        int32_t bbox_idx = 0;

        for (int32_t m = 0; m < num_wnd; m++) {				//detect all proposal windows with Coarse MLP and Fine MLP
          if (bboxes[m].bbox.x + bboxes[m].bbox.width <= 0 ||
              bboxes[m].bbox.y + bboxes[m].bbox.height <= 0)
            continue;
          GetWindowData(img, bboxes[m].bbox);
          feat_map->Compute(wnd_data_.data(), wnd_size_, wnd_size_);		//window compute. window data in wnd_data.data()
          feat_map->SetROI(roi);
        ...
        proposals[buf_idx[j]].resize(bbox_idx);		//now only bbox_idx+1 bboxs survived.

        if (k < num_stage_[cls_idx] - 1) {			//not the last stage
          ...
          if (i == num_hierarchy_ - 1) {			//the Fine MLP
            seeta::fd::NonMaximumSuppression(&(proposals[buf_idx[j]]),
